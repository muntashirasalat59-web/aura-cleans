-- =============================================
-- Step 2: RPC functions (run AFTER supabase.schema.sql)
-- Supabase SQL Editor: paste and run THIS ENTIRE FILE in one go
-- (Do not run only half of a function — select all lines below)
-- =============================================
...
BEGIN
  ...
  -- Purchase from supplier: reduce balance (negative = payable to party)
  UPDATE parties
...
GRANT EXECUTE ON FUNCTION public.create_purchase(BIGINT, DATE, TEXT, NUMERIC, JSONB) TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.create_sale(BIGINT, TEXT, DATE, NUMERIC, JSONB) TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.update_sale(BIGINT, BIGINT, DATE, NUMERIC, JSONB) TO anon, authenticated, service_role;

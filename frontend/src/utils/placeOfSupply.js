...
export function stateFromAddress(address) {
  ...
  if (!raw.trim()) return '';
  const found = STATE_NAMES.find((name) => new RegExp(`\\b${name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i').test(raw));
  return found || '';
}

export function derivePlaceOfSupply(party = {}) {
  return stateFromGstin(party.gst_number) || stateFromAddress(party.address) || '';
}

export function shippingIsSameAsBilling(shippingAddress, billingAddress) {
